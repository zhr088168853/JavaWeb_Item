...
public class AddRoomServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (room_num.equals("") | room_tel.equals("") | room_people.equals("")  | room_man.equals("")) {  //填写信息不能为空
            response.sendRedirect("Room/add_room.jsp");
        }
        else {
            ...
            if (zhr) {      //房间号不重复则执行插入操作
                ...
                if (flag) {       // 当操作成功时，转发到success.jsp页面
                    request.getRequestDispatcher("Room/success.jsp").forward(request, response);
                } else {      // 否则，转发到error.jsp页面，显示错误信息！
                    response.sendRedirect("Room/error.jsp");
                }
            } else {      //房间号有重复则提示错误
                request.getRequestDispatcher("Room/Duplication.jsp").forward(request, response);
            }
        }
    }
