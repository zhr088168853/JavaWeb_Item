...
@WebServlet("/AddStuServlet1")
public class AddStuServlet1 extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (sno.equals("") | stu_name.equals("") | stu_class.equals("")  | room_num.equals("")  | maintain.equals("")  | off_time.equals("")  | late_time.equals("")) {  //填写信息不能为空
            response.sendRedirect("student/add_stu1.jsp");
        }
        else {
                ...
                if (isroom){    //存在宿舍房间号才能入住
                    ...
                    if(zhang){      //宿舍没有住满才可以分配入住
                        ...
                        if (flag) {       // 当操作成功时，转发到success.jsp页面
                            request.getRequestDispatcher("student/Stu_success1.jsp").forward(request, response);
                        } else {      // 否则，转发到error.jsp页面，显示错误信息！
                            response.sendRedirect("student/Stu_error1.jsp");
                        }
                    } else {
                        response.sendRedirect("student/DonLive1.jsp");//分配的宿舍已住满
                    }
                }else {
                    response.sendRedirect("student/RoomNotExist1.jsp");//宿舍房间号不存在
                }
            } else {      //学号有重复则提示错误
                request.getRequestDispatcher("student/StuDuplication1.jsp").forward(request, response);
            }
        }
    }
