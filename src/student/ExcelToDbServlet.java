...
@WebServlet("/ExcelToDbServlet")
public class ExcelToDbServlet extends HttpServlet {//一键实现将Excel中的数据导入数据库
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String filename = request.getParameter("file");
        //得到Excel表格中所有的数据
        List<StuInfo> listExcel=StuService.getAllByExcel("d://"+filename);
        if (listExcel==null){       //找不到Excel文件
            response.sendRedirect("student/ToDBError.jsp");
        }
        else {
            DBCon db=new DBCon();
            ...
            response.sendRedirect("student/ToDBSuccess.jsp");  //跳转到成功页面
        }
    }
