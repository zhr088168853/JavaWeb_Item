    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (list==null){        //找不到文件，文件名有误
                response.sendRedirect("student/ToExcelError.jsp");
            }
            else {
                ...
                // 关闭Excel工作簿对象
                wwb.close();
                response.sendRedirect("student/ToExcelSuccess.jsp");    //成功导入到excel文件
            }
        } catch(FileNotFoundException e){   //当student.xls文件没有关闭时提示错误
            response.sendRedirect("student/NotFoundFile.jsp");
        }
